...
public class QueueServiceUtils {
	...
	public static QNameValue[] transformEntryVariables(List<com.vignette.idm.server.NameValue> aEntryVariables) {
		if (aEntryVariables == null) {
			return null;
		} else {
			QNameValue[] entryVariables = new QNameValue[aEntryVariables.size()];
			for (int i = 0; i < aEntryVariables.size(); i++) {
				entryVariables[i] = new QNameValue(aEntryVariables.get(i).getName(), aEntryVariables.get(i).getValue());
			}
			return entryVariables;
		}
	}

	public static SearchResult transformSelectionResult(SelectionResult aSelectionResult, int aMaxRows) {
	...
	public static ArrayList<com.vignette.idm.server.QueueEntry> transformQueueEntryList(QueueEntry[] aQueueEntryList) {
		ArrayList<com.vignette.idm.server.QueueEntry> queueEntryList = new ArrayList<com.vignette.idm.server.QueueEntry>(
				aQueueEntryList.length);
	...
	public static com.vignette.idm.server.QueueEntry transformQueueEntry(QueueEntry aQueueEntry) {
		ArrayList<com.vignette.idm.server.NameValue> variables = new ArrayList<com.vignette.idm.server.NameValue>(
				aQueueEntry.m_Variables.length);
	...
	public static com.vignette.idm.server.QueueInfo transformQueueInfo(QueueInfo aQueueInfo) {
		int queueVarDefsLen = aQueueInfo.m_RequiredVarDefs.length;
		ArrayList<com.vignette.idm.server.QueueVarDef> queueVarDefs = new ArrayList<com.vignette.idm.server.QueueVarDef>(
				queueVarDefsLen);
	...
	public static ArrayList<com.vignette.idm.server.QueueInfo> transformQueueInfoList(QueueInfo[] aQueueInfoList) {
		ArrayList<com.vignette.idm.server.QueueInfo> queueInfoList = new ArrayList<com.vignette.idm.server.QueueInfo>(
				aQueueInfoList.length);
	...
	public static void validateQueueIDs(String aMethodName, List<Long> aQueueIDs) throws InvalidParameterException {
		...
		for (int i = 0; i < aQueueIDs.size(); i++) {
			Validator.validateLongRangeParam(SERVICENAME, aMethodName, QUEUEIDS + "[" + i + "]", aQueueIDs.get(i), 1,
	...
	public static com.vignette.idm.server.QueueDataType transformDataType(int aDataType) {
