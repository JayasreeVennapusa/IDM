...
public class DocServiceUtils {
	...
	public static void checkPermission(String aMethodName, IdmRepImpl aRepo, String aAppName, String aDocID,
			int aAction) throws PermissionException, TTException {
		DoctypeSecurity docTypeSec = new DoctypeSecurity(aRepo);
		boolean perm = docTypeSec.validateIfnDoctypeAccess(aAction, aAppName, aDocID);
		if (perm == false) {
	...
	public static ArrayList<com.vignette.idm.server.RenditionDetail> transformRenditionDetails(
		...
		ArrayList<com.vignette.idm.server.RenditionDetail> details = new ArrayList<com.vignette.idm.server.RenditionDetail>(
				rends.length);
	...
	public static String validateRenditionType(String aMethodName, String aRenditionType)
			throws InvalidParameterException {
		// Perform general string validation.
		String renditionType = Validator.validateString(SERVICENAME, aMethodName, RENDITIONTYPE, aRenditionType, true,
				true);
	...
	public static void validateGetRenditionOptions(String aMethodName, List<com.vignette.idm.server.NameValue> aOptions)
		...
		if (aOptions != null) {
			...
			while (itor.hasNext()) {
				...
				if (optionName.equals(HEIGHT)) {
					...
					Validator.validateLongRangeParam(SERVICENAME, aMethodName, optionName, mHeight, 0, 32767);
				}
				// Check for valid values of Width option.
					...
					Validator.validateLongRangeParam(SERVICENAME, aMethodName, optionName, mWidth, 0, 32767);
				}
				// Check for valid values of ZoomFactor option.
					...
					Validator.validateLongRangeParam(SERVICENAME, aMethodName, optionName, mZoomFactor, 0, 9999);
				}
				// Check for valid values of Force8Bits option.
				else if (optionName.equals(FORCE8BITS)) {
					if (optionValue.equalsIgnoreCase("Y") == false && optionValue.equalsIgnoreCase("N") == false) {
				...
				// Check for valid values of ConvType option.
					...
					Validator.validateLongRangeParam(SERVICENAME, aMethodName, optionName, mConvType, 0, 2);
				}
				...
				// Check for valid values of FontScalingFactor option.
					...
					Validator.validateLongRangeParam(SERVICENAME, aMethodName, optionName, mFontScalingFactor, 0, 999);
				}
	...
	public static NameValue[] transformOptions() {
		...
		if (mHeight >= 0) {
			option = new NameValue(HEIGHT.toUpperCase(), Integer.toString(mHeight));
			optionsList.add(option);
		}
		if (mWidth >= 0) {
			option = new NameValue(WIDTH.toUpperCase(), Integer.toString(mWidth));
			optionsList.add(option);
		}
		if (mZoomFactor >= 0) {
			option = new NameValue(ZOOMFACTOR.toUpperCase(), Integer.toString(mZoomFactor));
			optionsList.add(option);
		}
		if (mForce8Bits != null) {
		...
		if (mConvType >= 0) {
			option = new NameValue(CONVTYPE.toUpperCase(), Integer.toString(mConvType));
			optionsList.add(option);
		}
		if (mColorOn != null) {
		...
		if (mFontScalingFactor >= 0) {
			option = new NameValue(FONTSCALINGFACTOR.toUpperCase(), Integer.toString(mFontScalingFactor));
			optionsList.add(option);
		}
		if (mHeightWidthOverwrite != null) {
			option = new NameValue(HWOVERWRITE.toUpperCase(), mHeightWidthOverwrite);
			optionsList.add(option);
		}
	...
	public static void validateSetAnnotationOptions(String aMethodName,
