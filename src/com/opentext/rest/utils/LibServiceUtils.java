...
public class LibServiceUtils {
	...
	public static void validateFields(String aMethodName, List<com.vignette.idm.server.NameValue> aFields)
			throws InvalidParameterException {
		// Perform general name/value pair validation.
		Validator.validateNameValueList(SERVICENAME, aMethodName, FIELDS, aFields, true, true, true, true);
		...
		if (aFields != null) {
			...
			while (itor.hasNext()) {
				...
				if (fieldName.equalsIgnoreCase(HASH_DOCID) || fieldName.equalsIgnoreCase(RECID)
						|| fieldName.equalsIgnoreCase(IFNDS) || fieldName.equalsIgnoreCase(IFNID)
						|| fieldName.equalsIgnoreCase(NPAGES) || fieldName.equalsIgnoreCase(DMID)
						|| fieldName.equalsIgnoreCase(DMVNUM) || fieldName.equalsIgnoreCase(DMRNUM)
						|| fieldName.equalsIgnoreCase(DMREVISE) || fieldName.equalsIgnoreCase(DMCO)
						|| fieldName.equalsIgnoreCase(DMCOUSER) || fieldName.equalsIgnoreCase(DMCODATE)
						|| fieldName.equalsIgnoreCase(DMCOTIME) || fieldName.equalsIgnoreCase(DMSTATUS)) {
	...
	public static void validateOptions(String aMethodName, List<com.vignette.idm.server.NameValue> aOptions)
	...
	public static NameValue[] transformFields(List<com.vignette.idm.server.NameValue> aFields) {
		if (aFields != null) {
			int i = 0;
	...
	public static void validateNoOptions(String aMethodName, List<com.vignette.idm.server.NameValue> aOptions)
			throws InvalidParameterException, UnknownOptionNameException, UnknownOptionValueException {
		// Perform general name/value pair validation.
		Validator.validateNameValueList(SERVICENAME, aMethodName, OPTIONS, aOptions, true, true, false, false);
	...
	public static ArrayList<String> transformDocIDs(NameValue[][] aRows) {
		...
		for (int i = 0; i < aRows.length; i++) {
			DocId ifn = new DocId(aRows[i][0].getValue(), aRows[i][1].getValue());
	...
	public static void validateFieldNames(String aMethodName, List<String> aFieldNames)
		...
		while (itor.hasNext()) {
			...
			// Check for unrecognised special field names.
					(fldName.startsWith("#") && fldName.equalsIgnoreCase(HASH_DOCID) == false)) {
				InvalidParameterExceptionFaultDetail faultDetail = new InvalidParameterExceptionFaultDetail();
				faultDetail.setCallStack(null);
				faultDetail.setMessage(null);
	...
	public static String validateCondition(String aMethodName, String aCondition) throws InvalidParameterException {
		// Perform general string validation.
		String condition = Validator.validateString(SERVICENAME, aMethodName, CONDITION, aCondition, true, false);
		...
		if (condition != null && condition.length() != 0) {
			...
			while (tokenizer.hasMoreTokens()) {
				...
				} else if (token.startsWith("#") || token.equalsIgnoreCase(DMID)) {
					InvalidParameterExceptionFaultDetail faultDetail = new InvalidParameterExceptionFaultDetail();
					faultDetail.setCallStack(null);
					faultDetail.setMessage(null);
	...
	public static String validateOrderBy(String aMethodName, String aOrderBy) throws InvalidParameterException {
		// Perform general string validation.
	...
	public static String getDMID(RecordCollection aCollection, String aAppName, String aDocID) throws TTException {
		DocId ifn = new DocId(aDocID);
		SQLStatement statement = new SQLStatement(SELECT + " " + DMID + " " + FROM + " " + aAppName + " " + WHERE + " "
				+ IFNDS + " = " + ifn.getDiskset() + " " + AND + " " + IFNID + " = " + ifn.getID());
		NameValue[][] rows = aCollection.executeQuery(statement, 0, 0);
		return rows[0][0].getValue();
	}

	public static SQLStatement constructSelect(String aTableName, List<String> aFields, String aDMID, String aCondition,
			String aOrderBy, ColumnDef aColumns) {
		SQLStatement statement = new SQLStatement(SELECT + " ");
		constructSelectFieldList(statement, aFields);
		statement.append(" " + FROM + " " + aTableName);
		constructWhereClause(statement, aDMID, aCondition, aColumns);
	...
	public static void constructWhereClause(SQLStatement aStatement, String aDMID, String aCondition,
			ColumnDef aColumns) {
	...
	public static SearchResult transformSearchResult(NameValue[][] aResult, int aMaxRecords) {
